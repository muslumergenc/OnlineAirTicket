    ...
    class AirBookTest
    {
        internal AirCreateReservationRsp AirBook(AirService.AirPricingSolution lowestPrice, AirService.AirItinerary airItinerary)
        {
            AirCreateReservationReq reservationReq = new AirCreateReservationReq
            {
                ...
                ContinuityCheckOverride = new ContinuityCheckOverride()
                {
                    Key = "1T",
                    Value = "true"
                },
                BookingTraveler = AddBookingTraveler(airItinerary),
                FormOfPayment = AddFormOfPayment(),
                AirPricingSolution = AddAirPriceSolution(lowestPrice, airItinerary),
            ...
            ActionStatus actionStatus = new ActionStatus()
            {
                ...
                TicketDate = reservationReq.AirPricingSolution[0].AirPricingInfo[0].LatestTicketingTime
            };
        ...
        private AirPricingSolution[] AddAirPriceSolution(AirService.AirPricingSolution lowestPrice, AirService.AirItinerary airItinerary)
        {
            ...
            foreach (var priceInfo in lowestPrice.AirPricingInfo)
            {
                ...
                {
                    info.ChangePenalty = new[]
                    {
                     new typeFarePenalty()
                     {Amount = priceInfo.ChangePenalty[0].Amount}
                    };
                }
            ...
            {
                finalPrice[i].AirPricingInfo = finalPriceInfo.ToArray();
                finalPrice[i].AirSegment = finalSegments.ToArray();
            }
            return finalPrice;
        }
